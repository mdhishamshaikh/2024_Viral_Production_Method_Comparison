...
# 
lr_24<- readxl::read_xlsx("./data/vp_assays/lm_vp_assays/cns_ons_ccs_vp_assays_lm.xlsx") %>%
...
write.csv(all_vp, file = "./results/vp_assays/vp_assay_comparison/viral_production_rates_cns_ons_ccs_lm_vipcla_vipcal_se_ngte.csv")



lytic_lm_vs_vpcl_vs_vpcl_se_vpcl_se_gte<- ggplot(all_vp  %>% dplyr::filter(Sample_Type == "Lytic") ,  
...
lytic_lm_vs_vpcl_vs_vpcl_se_vpcl_se_gte

ggsave(plot = lytic_lm_vs_vpcl_vs_vpcl_se_vpcl_se_gte + theme(legend.position = 'none'), filename = "./figures/lytic_lm_vs_vpcl_vs_vpcl_se_vpcl_se_gte.png", dpi = 800, width = 200, height = 70, units = "mm")
ggsave(plot = lytic_lm_vs_vpcl_vs_vpcl_se_vpcl_se_gte, filename = "./figures/lm_vs_vpcl_vs_vpcl_se_vpcl_se_gte.svg", dpi = 800, width = 220, height = 200, units = "mm")




lysogenic_lm_vs_vpcl_vs_vpcl_se_vpcl_se_gte<- ggplot(all_vp  %>% dplyr::filter(Sample_Type == "Lysogenic") ,  
                                                 aes(x = as.factor(Station_code), y = VP/1e+6, fill = VP_Method)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.7, color = "black") +
  geom_errorbar(aes(ymin = (VP - VP_SE)/1e+6, ymax = (VP + VP_SE)/1e+6),
                position = position_dodge(width = 0.7), width = 0.25) +
  ...
  labs(#title = "NJ2020 Viral Production",
    ...
    fill = "Analytical\napproach") +
  scale_fill_manual(values = c("LR" = "#DC2828", "VIPCAL" = "#003049", "VIPCAL-SE" ="#2A9D8F", "VIPCAL-SE-GTE" ="#F6CD61")) +
  scale_y_continuous(expand = expansion(mult = c(0.02, 0.0))) +
  #facet_wrap(~ Sample_Type, ncol = 1, scales = "free") + 
  theme_classic(base_size = 12) +
  theme(strip.background = element_blank(),
        axis.text.x = element_text(angle = 90),
...
lysogenic_lm_vs_vpcl_vs_vpcl_se_vpcl_se_gte

ggsave(plot = lysogenic_lm_vs_vpcl_vs_vpcl_se_vpcl_se_gte + theme(legend.position = 'none'), filename = "./figures/lysogenic_lm_vs_vpcl_vs_vpcl_se_vpcl_se_gte.png", dpi = 800, width = 200, height = 70, units = "mm")
