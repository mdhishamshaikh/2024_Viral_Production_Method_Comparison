...
work_dir<- getwd()
work_dir
setwd(work_dir) #There are some issues with the right path. Make sure it is the right working directory before moving on
```
...
```{r}
source(paste0(work_dir,"./scripts/3_nj2020_importing_fcs_and_count_source.R"))
set_up_vp_count()
```
